...
const getGptPromptBase = async () => {
  return (
    ...
    `Human: Could you explain what AI is?\n` +
    `AI: AI is short for artificial intelligence. It is the intelligence of machines and the software that powers them.\n`
  )
}
...
export async function generateAnswersWithGptCompletionApi(
  ...
  const prompt =
    ...
    getNonChatGptConversationPairs(session.conversationRecords || []) +
    `Human:${question}\nAI:`
  const apiUrl = (await getUserConfig()).OpenAiApiUrl

  let answer = ''
  await fetchServerSentEvents(`${apiUrl}/v1/completions`, {
    method: 'POST',
...
export async function generateAnswersWithChatgptApi(
  port: Browser.Runtime.Port,
  question: string,
  session: Session,
  apiKey: string,
  ...
  const prompt = getChatGptConversationPairs(session.conversationRecords || [])
  prompt.unshift({ role: 'system', content: await getChatgptPromptBase() })
  prompt.push({ role: 'user', content: question })
  const apiUrl = (await getUserConfig()).OpenAiApiUrl

  let answer = ''
  await fetchServerSentEvents(`${apiUrl}/v1/chat/completions`, {
    method: 'POST',
    signal: controller.signal,
    headers: {
      'Content-Type': 'application/json',
